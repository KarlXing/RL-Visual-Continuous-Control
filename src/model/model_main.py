...
class CURL_Model(nn.Module):
    def __init__(self, obs_shape, action_shape, hidden_dim, 
        ...
        self.actor = Actor(obs_shape, action_shape, hidden_dim, 
                           encoder_feature_dim, log_std_min, log_std_max, num_layers, num_filters).to(device)
        ...
        self.curl = CURL(encoder_feature_dim, self.critic, self.critic_target, output_type='continuous').to(device)
        self.device = device
